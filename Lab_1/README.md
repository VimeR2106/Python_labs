...
3. Добавлены тесты в `tests/`, проверяющие корректность работы функций.
4. Настроен `pytest` для запуска тестов через `python3 -m pytest`.
5. Проведено тестирование и исправлены выявленные ошибки.
6. Отчет оформлен в этом файле `README.md`.
...
> **Примечание:** Для подробностей используйте `git --help`.
